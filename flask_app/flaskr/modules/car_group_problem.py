...
class CarGroupProblem:
    ...
    def _formulate(self):
        ...
        prob = mip.Model(self.name, sense=mip.MINIMIZE)
        ...
        x = {
            (s, c): prob.add_var(name=f"x_{s}_{c}", var_type=mip.BINARY) for s, c in SC
        }
    ...
    def solve(self):
        ...
        solution_df = pd.DataFrame(
            list(student2car.items()), columns=["student_id", "car_id"]
        )
